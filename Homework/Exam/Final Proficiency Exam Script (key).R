...
# If you're interested (skip if not), the theory is that those students who can accurately PREdict and POSTdict their
...
# ID - gereric ID number to replace names
# Gender - m or f designation only available (no students identified otherwise)
# Prediction1, Prediction2, Prediction3 - PREdicted scores on Exams 1, 2, and 3, percent
# Postdiction1, Postdiction2, Postdiction3 - POSTdiction scores on Exams 1, 2, and 3, percent
# Grade1, Grade2, Grade3 - Actual grade recieved on Exams 1, 2, and 3, percent
# FinalScore - Overall grade (percent points possible) in the class, percent

# #1 ----------------------------------------------------------------------

# Import the data and change all "f" to "Female" and "m" to "Male". Also, write
# over the existing data for all grades so they are a percent (out of 100) instead
# of a proportion out of 1. You can find the data here:
# https://raw.githubusercontent.com/jordanharshman/R22/main/data/Predictions.csv
pred <- read_csv(url("https://raw.githubusercontent.com/jordanharshman/R22/main/data/Predictions.csv")) %>%
  mutate(Gender = ifelse(Gender == "f", "Female", "Male"),
         across(contains("Grade"), ~ .x * 100))

# #2 ----------------------------------------------------------------------
# Across all 3 exams, how many missing predictions were there? Just looking for a
# single number; don't need to know how many per exam. 
pred %>%
  select(contains("Predict")) %>%
  is.na() %>%
  sum()

# My Answer (1 number): 66
...
pred %>% 
  select(-ID:-Gender, -FinalScore) %>%
  pivot_longer(everything(), names_to = "Estimate") %>%
  group_by(Estimate) %>%
  summarize(Avg = mean(value, na.rm = T))

# My Answers (3 orders):
# Exam 1 order: Pre > Post > Grade
# Exam 2 order: Grade > Pre > Post
# Exam 3 order: Pre > Grade > Post
...
# Hints: There is a lot to do here both behind the scenes and in the plot itself. 

# Hint 1: Normally, when you make a facet plot(), you need to pivot_longer() first. While that is kind of true
# here (you will likely want to pivot eventually), it won't be so straightfoward. If you pivot, you'll get all
# Pred1, Pred2... Post1, Post2... Grade1, Grade2... stacked on top of each other, but what you really need is a
# tibble that has 4 columns: Prediction or Postdiction (x), Grade (y), a variable that identifies if someone is
# within 10% of their prediction/postdiction (color), and Exam (facet); pivot_longer() won't directly return this,
...
# consider turning the following logic into a mutate() call:
...
pred.l <- pred1 %>%
  rbind(pred2, pred3) %>%
  pivot_longer(c(Prediction, Postdiction), "_diction") %>%
  rowwise() %>%
  mutate(In10 = if_else(between(Grade, (value - 10), (value + 10)), "in", "out"))

ggplot(pred.l, aes(x = value, y = Grade, color = In10)) +
  geom_jitter(height = 1, width = 1, alpha = .6) +
  geom_abline(slope = 1, intercept = 0, lty = 1) +
  geom_abline(slope = c(1,1), intercept = c(-10,10), lty = 2) +
  facet_wrap(`_diction`~Exam) +
  scale_color_manual(values = c("#dd550c", "#03244d")) +
  scale_x_continuous(limits = c(50, 103), name = "Pre/Postdiction") +
  scale_y_continuous(limits = c(50, 103)) +
  theme_classic() +
  theme(legend.position = "none")

ggsave("Predictions.pdf", height = 4.5, width = 6)
